...
type (
	friendRequestsModel interface {
		...
		FindByUserIdAndReqId(ctx context.Context, uid, reqId string) (*FriendRequests, error)
...
func (m *defaultFriendRequestsModel) FindByUserIdAndReqId(ctx context.Context, uid, reqId string) (*FriendRequests, error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ? and `req_uid` = ?",
		friendRequestsRows, m.table)
	var resp FriendRequests
	err := m.QueryRowNoCacheCtx(ctx, &resp, query, uid, reqId)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
