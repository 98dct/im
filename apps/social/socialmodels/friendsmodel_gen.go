...
type (
	friendsModel interface {
		...
		Inserts(ctx context.Context, session sqlx.Session, datas ...*Friends) (sql.Result, error)
...
func (m *defaultFriendsModel) ListByUserId(ctx context.Context, userId string) ([]*Friends, error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ?",
		friendsRows, m.table)
	var resp []*Friends
...
func (m *defaultFriendsModel) Inserts(ctx context.Context, session sqlx.Session, datas ...*Friends) (sql.Result, error) {
	var (
		sql  strings.Builder
		args []interface{}
	)
	...
	for i, data := range datas {
		sql.WriteString("(?, ?, ?, ?)")
		args = append(args, data.UserId, data.FriendUid, data.Remark, data.AddSource)
